...
export function albumSlug(artist: string, album: string): string {
...
export async function generateUniqueSlug(
  ...
  const base = albumSlug(artist, album);
  let slug = base;
  let n = 2;
  while (true) {
    const q = excludeId
      ? db.prepare('SELECT id FROM albums WHERE slug = ? AND id != ?').bind(slug, excludeId)
      : db.prepare('SELECT id FROM albums WHERE slug = ?').bind(slug);
...
// Returns the uppercase ISO 3166-1 alpha-2 code for a stored country value.
// Handles both full names ("France") and codes already stored ("FR").
export function countryCode(country: string): string {
  if (!country?.trim()) return '';
  const t = country.trim();
  if (/^[A-Za-z]{2}$/.test(t)) return t.toUpperCase();
  const found = COUNTRIES.find(c => c.name.toLowerCase() === t.toLowerCase());
  return found ? found.code : '';
}

// Returns the full country name for an ISO code. Used to convert MusicBrainz ISO codes
// to the human-readable names stored in the DB.
