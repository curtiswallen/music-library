...
export const GET: APIRoute = async (context) => {
  ...
  try {
    ...
    const [artistRes, titleRes] = await Promise.all([
      fetch(`${MB}/release-group?query=artist:${enc(`"${q}"`)}&fmt=json&limit=8&inc=genres`, { headers: HDR }),
      fetch(`${MB}/release-group?query=${enc(q)}&fmt=json&limit=5&inc=genres`,               { headers: HDR }),
    ]);
